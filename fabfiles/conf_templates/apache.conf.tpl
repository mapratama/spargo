<VirtualHost *>
    ...
    Alias /static/ %(project_path)ssrc/static/
	...
	<Directory %(project_path)ssrc/media>
        Order deny,allow
        Allow from all				
    </Directory>
	...
	WSGIScriptAlias / %(project_path)ssrc/apache/django.wsgi
    <Directory %(project_path)slatest/apache>
        Order Allow,Deny
        Allow from all
    </Directory>
    ErrorLog  %(project_path)slogs/%(server_name)s.error.log
    CustomLog %(project_path)slogs/%(server_name)s.access.log combined
</VirtualHost>
